...
class Directory implements Comparable {
    ...
    public static Directory[] getRootDirectories(){
        ...
        if (isWindows){
            ...
            if (doNetUse){
                ImageModule.Image.Shell cmd = new ImageModule.Image.Shell("net use");
                cmd.run();
    ...
    public boolean exists(){
        ...
        if (isWindows && path.startsWith("\\\\")){
            ...
            if (!path.substring(2).contains(PathSeparator)){
                ...
                if (doNetUse){
                    ImageModule.Image.Shell cmd = new ImageModule.Image.Shell("net view " + path);
                    cmd.run();
        ...
        while (true){
            ...
            synchronized (results) {
                ...
                if (item!=null){
                    ...
                    if (item instanceof ImageModule.Image.File){

                        ImageModule.Image.File file = (ImageModule.Image.File) item;
                        ...
                        ImageModule.Image.Directory dir = (ImageModule.Image.Directory) item;
    ...
    public File[] getFiles(Object filter){
        ...
        if (this.exists()){
            ...
            if (files==null){
                ...
                java.util.ArrayList<ImageModule.Image.File> list = new java.util.ArrayList<ImageModule.Image.File>();
                for (int i=0; i<files.length; i++){
                    ...
                    if (file.exists() && file.isFile()){
                        list.add(new ImageModule.Image.File(file));
                    }
                }
                Collections.sort(list, new FileComparer(this));
                return list.toArray(new ImageModule.Image.File[list.size()]);
            }
        }
        else{
            return new File[0];
    ...
    private java.io.File[] getSharedDrives(String serverName){
        if (isWindows){
            ...
            if (doNetView){
                ...
                ImageModule.Image.Shell cmd = new ImageModule.Image.Shell("net view " + serverName);
                cmd.run();
                java.util.List errors = cmd.getErrors();
                errors.remove(null);
                if (errors.isEmpty()){
    ...
    private String[] dir(){
        ...
        if (doDir)
            try{
                ...
                ImageModule.Image.Shell cmd = new ImageModule.Image.Shell("cmd.exe /c dir /OG " + path);
                java.util.List<String> output = cmd.getOutput();
                cmd.run(true);
